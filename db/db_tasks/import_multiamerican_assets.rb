...
blog_entries.each_with_index do |blog_entry, bindex|
  ...
  post = MultiAmerican::Post.find.find { |p| p.id == blog_entry.wp_id }
  
  if !post.present?
    puts "#{boutof} No MultiAmerican::Post with id #{blog_entry.wp_id}"
    next
  end
  ...
  if fake_assets.blank?
    puts "#{boutof} No fake assets for MultiAmerican::Post ##{post.id}"
    next
  end
  ...
  fake_assets.each_with_index do |asset, aindex|
    begin
      ...
      # Owner
      # Try to use what's stored in the MultiAmerican::Attachment object
      ...
      # what's in the MultiAmerican::Attachment excert attribute
      ...
      asset_desc = "MultiAmerican::Attachment ##{attachment.id}  as Asset ##{new_asset['id']}  " \
        "from MultiAmerican::Post ##{post.id}  to BlogEntry ##{blog_entry.id}"
      ...
      content_asset = ContentAsset.new(
        django_content_type_id: MultiAmerican::PostBase::CONTENT_TYPE_ID,
        content:                blog_entry,
        asset_order:            aindex,
