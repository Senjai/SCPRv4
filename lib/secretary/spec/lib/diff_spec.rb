...
describe Secretary::Diff do
  ...
  describe "::should_ignore" do
    before :each do
      stub_const("Secretary::Diff::DEFAULTS", "id")
    end
    
    it "returns true if IGNORES includes the passed-in string" do
      Secretary::Diff.should_ignore("id").should be_true
    end
    
    it "returns false if IGNORES does not include the passed-in string" do
      Secretary::Diff.should_ignore("body").should be_false
    end  
  end
end
