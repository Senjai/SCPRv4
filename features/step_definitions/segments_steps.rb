...
Then /^I should see each segment's primary asset$/ do
  page.should have_css ".teaser .contentasset img" # should put a count on this
end
...
Then /^I should see a list of that episode's segments$/ do
  page.should have_css ".teaser", count: @episode.segments.count
end
...
Then /^only the published segments should be displayed$/ do
  ...
  page.should have_css ".teaser", count: @episode.segments.published.count
